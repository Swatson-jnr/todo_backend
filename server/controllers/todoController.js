...
export const addTodo = async (req, res) => {
  try {
    ...
    const {  title, description, inProgress, dueDate, completed } = req.body;
    ...
    const newTodo = { title, description, inProgress, dueDate, completed };

    // push the new todo into user's todo array
    const user = await userModel.findByIdAndUpdate(userId, { $push: { todo: newTodo } },{ new: true } 
    );
    ...
    res.status(201).json({success: true, message: "Todo added successfully",todo: user.todo});
  } catch (error) {
...
export const updateTodo = async (req, res) => {
  try {
    ...
    if(!title || !description){
      return res.status(409).json({success: false, message: "missing fields, enter all required fields"})
    }
    ...
    res.status(200).json({success: true, message: "Todo updated successfully", todos: user.todo});
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
